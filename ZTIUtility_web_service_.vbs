...
class Logging
	...
	Function CreateEvent(iEventID, iType, sMessage, arrParms)
		...
		If oEnvironment.Item("EventService") <> "" then
			...
			vData = vData & "|userID^0|ram=" &  oEnvironment.Item("Memory") & "|cpu^" & oEnvironment.Item("cpu") & "|hostname^" & oEnvironment.Item("ComputerName") & "|installationIP^" & oEnvironment.Item("IpAddress001")
			vData = vData & "|firmware^" & oEnvironment.Item("firmware") & "|disksCount^" & oEnvironment.Item("disksCount") & "|NetIntCount^" & oEnvironment.Item("NetIntCount")
			...
			wsService.WebService = oEnvironment.Item("WebSysprod") & "?" & vData
