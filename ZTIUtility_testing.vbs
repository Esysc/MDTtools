		...
		If oEnvironment.Item("EventService") <> "" then
			...
			sLine = sLine & "&dartIP=" & oEnvironment.Item("DartIP001") & "&dartPort=" & oEnvironment.Item("DartPort001") & "&dartTicket=" & oEnvironment.Item("DartTicket") & "&vmHost=" & oEnvironment.Item("VMHost") & "&vmName=" & oEnvironment.Item("VMName")
			 ...
			 ' Add additional parameters specified in MonitorProperties
                         ...
                         Dim json
						json = "{"
						json = json & """salesorder"":""" & oEnvironment.Item("So") & ""","
						json = json & """rack"":""" & oEnvironment.Item("Rack") & ""","
						json = json & """shelf"":""" & oEnvironment.Item("Shelf") & ""","
						json = json & """data"":""" & oEnvironment.Item("Data") & ""","
						json = json & """status"":""" & oEnvironment.Item("_SMSTSCurrentActionName") & ""","
						json = json & """progress"":""" & Round(sCurrentStep / sTotalSteps * 100) & ""","
						json = json & """model"":""" & oEnvironment.Item("Model") & ""","
						json = json & """serial"":""" &  oEnvironment.Item("SerialNumber") & ""","
						json = json & """userID"":""1"","
						json = json & """ram"":""" & oEnvironment.Item("Memory") & ""","
						json = json & """cpu"":""" & oEnvironment.Item("cpu") & ""","
						json = json & """hostname"":""" & oEnvironment.Item("ComputerName") & ""","
						json = json & """installationIP"":""" & oEnvironment.Item("IpAddress001") & ""","
						json = json & """firmware"":""" & oEnvironment.Item("firmware") & ""","
						json = json & """diskscount"":""" & oEnvironment.Item("disksCount") & ""","
						json = json & """netintcount"":""" & oEnvironment.Item("NetIntCount") & """"
						json = json & "}"
		    ...
		    wsService.WebService = oEnvironment.Item("WebSysprod") & "/" & oEnvironment.Item("So") & "&json=" & json
			
			
			wsService.ContentType = "application/json"
            wsService.Method = "GET"
            wsService.Quiet = false
			wsService.AcceptCharset = """"
			wsService.UserAgent = "perl"
			wsService.AcceptLanguage = ""
			'wsService.Parameters = json
